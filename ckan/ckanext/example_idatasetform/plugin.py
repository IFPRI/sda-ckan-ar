...
def megasites():
    try:
        megasites = tk.get_action('tag_list')(
                data_dict={'vocabulary_id': 'megasites'})
        return megasites
    except tk.ObjectNotFound:
        return None
...
def countries():
    try:
        countries = tk.get_action('tag_list')(
                data_dict={'vocabulary_id': 'countries'})
        return countries
    except tk.ObjectNotFound:
        return None
...
def villages():
    try:
        villages = tk.get_action('tag_list')(
                data_dict={'vocabulary_id': 'villages'})
        return villages
    except tk.ObjectNotFound:
        return None
...
class ExampleIDatasetFormPlugin(plugins.SingletonPlugin,
    ...
    def get_helpers(self):
        return {'villages': villages, 'countries': countries, 'megasites': megasites}
...
#            })
        schema.update({
            'megasites': [
                tk.get_converter('convert_from_tags')('megasites'),
                tk.get_validator('ignore_missing')]
            })
        schema.update({
            'countries': [
                tk.get_converter('convert_from_tags')('countries'),
                tk.get_validator('ignore_missing')]
            })
        schema.update({
            'villages': [
                tk.get_converter('convert_from_tags')('villages'),
                tk.get_validator('ignore_missing')]
            })
	...
	log.error(facets_dict)
        return OrderedDict([('vocab_megasites', u'Megasites'),
			    ('vocab_countries',u'Countries'),			    
			    ('vocab_villages', u'Villages'),
                           ] + facets_dict.items())
